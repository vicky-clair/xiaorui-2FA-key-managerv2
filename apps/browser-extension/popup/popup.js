...
// node_modules/.bun/jsqr@1.4.0/node_modules/jsqr/dist/jsQR.js
...
var RFC4648_ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";
...
function parseOtpAuthUri(uri) {
  ...
  let algorithm = "SHA-1";
  if (rawAlgo === "SHA256" || rawAlgo === "SHA-256")
    algorithm = "SHA-256";
...
async function renderEntries() {
  ...
  for (const entry of filtered) {
    ...
    const progressColor = totp.remainingSeconds > 10 ? "#10b981" : totp.remainingSeconds > 5 ? "#f59e0b" : "#ef4444";
    const formattedCode = totp.code.length === 6 ? `${totp.code.slice(0, 3)} ${totp.code.slice(3)}` : totp.code;
    card.innerHTML = `
...
async function updateTotpCodes() {
  for (const entry of entries) {
    ...
    try {
      ...
      barEl.style.backgroundColor = totp.remainingSeconds > 10 ? "#10b981" : totp.remainingSeconds > 5 ? "#f59e0b" : "#ef4444";
    } catch {}
  }
}
...
var inputQrFile = document.getElementById("input-qr-file");
